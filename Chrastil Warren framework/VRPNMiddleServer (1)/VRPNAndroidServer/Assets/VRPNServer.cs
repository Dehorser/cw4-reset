...
public class VRPNServer : MonoBehaviour {
	...
	void Update () 
	{
		foreach (VRPNConnection vrpnC in _connectionList.Values) {
			if (vrpnC._VRPNObject != null) {
				...
				NetworkServer.SendToClient (1, MESSAGE_DATA, new VRPNMessage (pos, quat));
			}
		}
	}
	...
	void OnGUI()
	{
		...
		foreach (VRPNConnection vrpnC in _connectionList.Values) {
			if (vrpnC._VRPNObject != null) {
				string info = vrpnC._VRPNObject + " Pos: " + vrpnC._pos.ToString () + " Orientation: " + vrpnC._quat.eulerAngles;
