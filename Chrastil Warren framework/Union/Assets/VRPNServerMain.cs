...
public class VRPNMessageMain : MessageBase
{
	public Vector3 _pos;
	public Quaternion _quat;

	public VRPNMessageMain()
	{
		_pos = new Vector3 ();
		_quat = new Quaternion ();
	}

	public VRPNMessageMain(Vector3 pos, Quaternion quat)
	{
		_pos = pos;
		_quat = quat;
	}
...
public class VRPNInfoMain : MessageBase
{
	...
	public VRPNInfoMain(string VRPNObject)
	{
		_VRPNObject = VRPNObject;
		_IPAdress = "192.168.1.100";
	}

	public VRPNInfoMain(string VRPNObject, string IPAdress)
	{
		_VRPNObject = VRPNObject;
		_IPAdress = IPAdress;
	}
}
public class VRPNConnectionMain
{
	public Vector3 _pos;
	public Quaternion _quat;
	public string _VRPNObject;
...
public class VRPNServerMain : MonoBehaviour {
	const short MESSAGE_DATA = 880;
	const short MESSAGE_INFO = 881;
	private Dictionary<int, VRPNConnectionMain> _connectionList;
	...
	void Start () {
		if (Application.platform != RuntimePlatform.Android) {
			...
			_connectionList = new Dictionary<int, VRPNConnectionMain> ();
		} else {
	...
	void Update () 
	{
		foreach (VRPNConnectionMain vrpnC in _connectionList.Values) {
			if (vrpnC._VRPNObject != null) {
				Vector3 tmpPos = VRPNMain.vrpnTrackerPos (vrpnC._VRPNObject, 0);
				Quaternion tmpQuat = VRPNMain.vrpnTrackerQuat (vrpnC._VRPNObject, 0);
				...
				NetworkServer.SendToClient (vrpnC._connectionID, MESSAGE_DATA, new VRPNMessageMain (pos, quat));
			}
		}
	}
	...
	{
		VRPNInfoMain vrpnData = vrpnMsg.ReadMessage<VRPNInfoMain> ();
		VRPNConnectionMain vrpnC = _connectionList [vrpnMsg.conn.connectionId];
	...
	public void OnConnected(NetworkMessage netMsg)
	{
		...
		VRPNConnectionMain newVRPNConnection = new VRPNConnectionMain ();
...
/*public class MyNetworkServerMain : MonoBehaviour {
	...
	void Update () 
	{
		if (isAtStartup) {
			...
			Vector3 tmpPos = VRPNMain.vrpnTrackerPos ("GearVR@192.168.1.100", 0);
			Quaternion tmpQuat = VRPNMain.vrpnTrackerQuat ("GearVR@192.168.1.100", 0);
			Vector3 pos = new Vector3 (-tmpPos.x, tmpPos.z, -tmpPos.y);
			Quaternion quat = new Quaternion (tmpQuat.x, -tmpQuat.z, tmpQuat.y, tmpQuat.w);
			if (_connectionID > 0) {
